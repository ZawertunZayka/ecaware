        ...
        if (windowPointer == this.mc.getMainWindow().getHandle())
        {
            ...
            if (action == GLFW.GLFW_PRESS) {
                ...
                if (key == GLFW.GLFW_KEY_RIGHT_SHIFT && mc.currentScreen == null) {
                    if (EclipseFuntime.abGui == null) {
                        EclipseFuntime.abGui = new AutoBuyGui();
                    } else {
                        EclipseFuntime.abGui.open();
                    }
                    mc.displayGuiScreen(EclipseFuntime.abGui);
                }
                if (mc.currentScreen == null) {
                    if (key == EclipseFuntime.autoLoot.bind) {
                        if (EclipseFuntime.autoLoot.toggled) {
                            EclipseFuntime.eventBus.unregister(EclipseFuntime.autoLoot);
                            EclipseFuntime.autoLoot.toggled = false;
                        } else {
                            EclipseFuntime.eventBus.register(EclipseFuntime.autoLoot);
                            EclipseFuntime.autoLoot.toggled = true;
                        }
                        mc.ingameGUI.getChatGUI().printChatMessage(ITextComponent.getTextComponentOrEmpty("АвтоЛут " + (EclipseFuntime.autoLoot.toggled ? "включен." : "выключен.")));
                    }
                    if (key == EclipseFuntime.autoMyst.bind) {
                        if (EclipseFuntime.autoMyst.toggled) {
                            EclipseFuntime.eventBus.unregister(EclipseFuntime.autoMyst);
                            EclipseFuntime.autoMyst.toggled = false;
                        } else {
                            EclipseFuntime.eventBus.register(EclipseFuntime.autoMyst);
                            EclipseFuntime.autoMyst.toggled = true;
                        }
                        mc.ingameGUI.getChatGUI().printChatMessage(ITextComponent.getTextComponentOrEmpty("АвтоМист " + (EclipseFuntime.autoMyst.toggled ? "включен." : "выключен.")));
                    }
                    if (key == EclipseFuntime.casino.bind) {
                        if (EclipseFuntime.casino.toggled) {
                            EclipseFuntime.eventBus.unregister(EclipseFuntime.casino);
                            EclipseFuntime.casino.toggled = false;
                        } else {
                            EclipseFuntime.eventBus.register(EclipseFuntime.casino);
                            EclipseFuntime.casino.on();
                            EclipseFuntime.casino.toggled = true;
                        }
                        mc.ingameGUI.getChatGUI().printChatMessage(ITextComponent.getTextComponentOrEmpty("Казино " + (EclipseFuntime.casino.toggled ? "включено." : "выключено.")));
                    }
                }
            }
